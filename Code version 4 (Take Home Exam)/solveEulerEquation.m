...
for ixt=(T-1):-1:1                              % Loop from time T-1 to 1
    ...
    for ixA = 1:1:numPtsA                       % Loop through points on the asset grid
        ...
        for ixY = 1:1:numPtsY                   % Loop through points on the income grid
            ...
            lbA1 = Agrid(ixt + 1, 1);           % lower bound: assets tomorrow  % here see that lb conditional on A and Y coincides with unconditional lb (i.e. lb of assets grid)
            ubA1 = (A + Y - minCons)*(1+r);     % upper bound: assets tomorrow  % here see that ub conditional on A and Y does not coincide with unconditional ub, but it is more restrictive to guarantee cmin
        ...
        % additional step needed if there is uncertainty
